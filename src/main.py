...
def build_argparser():
    ...
    parser.add_argument(
        "-rp",
        "--reidentification_model_precision",
        type=str,
        default="FP32-INT8",
...
def add_emotion_emoji(face_emotion, cord, bbox_frame):
    ...
    n = cord[1] + cord[3] - 20
    o = cord[1] + cord[3]
    bbox_frame[n:o, l:m, :] = cv2.resize(
        cv2.imread(os.path.join(parent_dir, "bin", f"{emotions[face_emotion]}.png")),
        (20, 20),
    )
...
def plot_emotion_count(bbox_frame, emotions_count):
    ...
    bbox_frame[276:420, 504:720, 2] = np.where(
        x[:, :, 2] == 0, 255, bbox_frame[276:420, 504:720, 2]
    )
    bbox_frame[420:450, 534:564, :] = cv2.resize(
        cv2.imread(os.path.join(parent_dir, "bin", "neutral.png")), (30, 30)
    )
    bbox_frame[420:450, 568:598, :] = cv2.resize(
        cv2.imread(os.path.join(parent_dir, "bin", "happy.png")), (30, 30)
    )
    bbox_frame[420:450, 602:632, :] = cv2.resize(
        cv2.imread(os.path.join(parent_dir, "bin", "sadness.png")), (30, 30)
    )
    bbox_frame[420:450, 636:666, :] = cv2.resize(
        cv2.imread(os.path.join(parent_dir, "bin", "surprise.png")), (30, 30)
    )
    bbox_frame[420:450, 670:700, :] = cv2.resize(
        cv2.imread(os.path.join(parent_dir, "bin", "anger.png")), (30, 30)
    )
...
def inference(args):
    ...
    logging.info(
        "Precision(detection,emotions,reidentification): {0},{1},{2}".format(
            args.detection_model_precision,
            args.emotion_model_precision,
            args.reidentification_model_precision,
        )
    )
    ...
    face_reidentification = FaceReidentification(
        precision=args.reidentification_model_precision
    )
    face_reidentification.load_model()

    logging.info(
        "3 models load time: {0:.4f}sec".format(time.time() - model_load_start)
    )
    ...
    for frame in input_feeder.next_batch():
        ...
        for index, face in enumerate(faces):
            ...
            time_sheet["reidentification_infr"].append(
                time.time() - reidentification_infr_start
            )
    ...
    logging.info(
        "face detection model avg inference per frame: {0:.4f}sec".format(
            np.mean(time_sheet["detection_infr"])
        )
    )
    logging.info(
        "emotion recognition model avg inference per frame: {0:.4f}sec".format(
            np.mean(time_sheet["emotion_infr"])
        )
    )
    logging.info(
        "face reidentification model avg inference per frame: {0:.4f}sec".format(
            np.mean(time_sheet["reidentification_infr"])
        )
    )

    logging.info(
        "3 Model avg inference per frame: {0:.4f}sec".format(
            np.mean(time_sheet["infr_per_frame"])
        )
    )
    logging.info(
        "Total inference time: {0:.4f}sec".format(time.time() - total_infr_start)
    )
    logging.info(
        "====================================END==========================================\n"
    )
