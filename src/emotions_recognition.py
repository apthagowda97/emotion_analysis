...
class EmotionsRecognition:
    ...
    def get_model_path(self):
        ...
        model_path = os.path.join(
            parent_dir, "model", "intel", model_name, self.precision, model_name
        )
        return model_path
    ...
    def predict(self, face):
        ...
        self.net.start_async(
            request_id=0, inputs={self.input_name: self.preprocess_image}
        )
    ...
    def preprocess_output(self, outputs, face):
        return np.argmax(outputs[self.output_name].squeeze())
